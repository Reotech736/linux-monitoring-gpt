...
1. **Phase 7: 異常系の検証**
   Node Exporter・Prometheus Agentの停止、メトリクス欠損、閾値超過を安全に再現し、GrafanaとCustom GPTが推測せず状況を説明できることを確認する。
2. **Phase 8: メトリクスと可視化の拡張**
   systemd、Docker、filesystem、inode、ディスクI/O、ネットワーク、温度などから実際に必要な指標を選び、Node Exporterのcollectorまたは専用Exporter、Grafanaダッシュボード、固定PromQLの診断APIを段階的に拡張する。
3. **Phase 9: 通知と運用**
   到達不能、ディスク逼迫、高負荷などを通知し、AMP、API Gateway、Lambda、Cognitoを含む月次コストと保持期間を見直す。
4. **Phase 10: 複数ホスト**
   `host_id`を単に自由入力にせず、利用者・Cognitoグループ・閲覧可能ホストの対応を設計してから対象ホストを追加する。必要に応じてDynamoDBなどで認可対応表を管理する。

Phase 7を次の優先事項とし、その結果を踏まえてPhase 8の対象メトリクスを決める。
